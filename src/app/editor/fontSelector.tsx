...
export function FontSelector({
  ...
  value: Option;
  onChange: (value: Option & { isGoogleFont: boolean } | null) => void;
  optionStyles: CSSProperties;
}) {
  const [googleFontsLoaded, setGoogleFontsLoaded] = usePersistedState<string[]>(
    "googleFonts",
    [],
    true
  );
  const [fontChoices, setFontChoices] =
    useState<GroupedOption[]>(baseFontChoices);
  const effectiveFontChoices = useMemo(() => {
    ...
    return [
      ...
      {
        label: "Google Fonts",
        options: googleFontsLoaded.map((f) => ({ value: f, label: f })),
      },
    ];
  ...
  useEffect(() => {
    (async () => {
      ...
      choices[0].options = (
        await Promise.all(
          choices[0].options.map((f) =>
            isFontAvailable(f.value).then((a) => (a ? f : null))
          )
        )
      ).filter((a): a is Option => a !== null);
      const modernFontStackChoices: GroupedOption = {
        label: "Latin Modern Font Stacks",
        options: [],
      };
      modernFontStackChoices.options = (
        await Promise.all(
          ModernFontStacksChoices.map(async ({ name, values }) => {
            const firstAvailableValue = (
              await Promise.all(
                values.map((f) =>
                  isFontAvailable(f).then((a) => (a ? f : null))
                )
              )
            ...
            return firstAvailableValue
              ? ({
                  ...
                  label: `${name} (${firstAvailableValue})`,
                } as Option)
              : null;
          })
        )
  ...
  return (
    <>
      <CreatableSelect<Option, false, GroupedOption>
        ...
        onChange={(v) => onChange(v ? {...v, isGoogleFont: googleFontsLoaded.includes(v.value)} : v)}
        createOptionPosition="first"
        formatCreateLabel={(inputValue) => (
          <span style={{ ...optionStyles, fontFamily: inputValue }}>
            Use “{inputValue}”…
          </span>
        )}
        classNames={comboBoxClassesConfig}
        formatOptionLabel={(option) => (
          <span style={{ ...optionStyles, fontFamily: option.value }}>
            {option.label}
          </span>
        )}
        // menuIsOpen
      ...
      <GoogleFontsPicker
        addFont={(font) => {
          setGoogleFontsLoaded((f) => [...f, font]);
          onChange({ value: font, label: font, isGoogleFont: true });
        }}
      />
